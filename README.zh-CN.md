...
Gnalloy Pipeline 的 Memcached binary 协议对象与 ASCII 编解码辅助能力。

该模块位于 transport 之上、业务 handler 之下，负责把字节或 Gnalloy 消息转换成协议对象，并把出站协议对象转换回字节。它不打开 socket，也不拥有 EventLoop。

## 状态

- 导入路径：`gnalloy.org/codec-memcache`
- 仓库：`github.com/gnalloy/codec-memcache`
- 默认分支：`dev`
- 预览安装：`go get gnalloy.org/codec-memcache@dev`
- 许可证：Apache-2.0
...
本仓库负责：Gnalloy Pipeline 的 Memcached binary 协议对象与 ASCII 编解码辅助能力。
