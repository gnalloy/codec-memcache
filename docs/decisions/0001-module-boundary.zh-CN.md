# ADR-001：保持 codec-memcache 为独立 Gnalloy 模块
...
`codec-memcache` 作为 `gnalloy.org/codec-memcache` 发布，并拥有该职责：Gnalloy Pipeline 的 Memcached binary 协议对象与 ASCII 编解码辅助能力。
